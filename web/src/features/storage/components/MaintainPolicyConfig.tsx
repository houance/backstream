...
export function MaintainPolicyConfig({ form }: { form: UseFormReturnType<StorageCreateSchema> }) {
        ...
        <Stack gap="md">
            <Select
                ...
                onChange={(val) => {
                    ...
                    if (val !== 'custom') {
                        form.setFieldValue('checkSchedule.cron', val!)
                    }
                }}
            />
            {checkScheduleType === 'custom' && (
                <TextInput
                    ...
                    placeholder="* * * * * *"
                    {...form.getInputProps('checkSchedule.cron')}
                    withAsterisk
                />
            )}
            {checkScheduleType !== 'custom' && checkScheduleType !== 'manual' && (
                <Paper withBorder p="xs" bg="var(--mantine-color-gray-0)">
                    <Text size="xs" c="dimmed">Resulting Cron: <b>{form.values.checkSchedule.cron}</b></Text>
                </Paper>
            )}
            ...
            <Select
                ...
                onChange={(val) => {
                    ...
                    if (val !== 'custom') {
                        form.setFieldValue('pruneSchedule.cron', val!)
                    }
                }}
            />
            {pruneScheduleType === 'custom' && (
                <TextInput
                    label="Cron Expression"
                    placeholder="* * * * * *"
                    {...form.getInputProps('pruneSchedule.cron')}
                    withAsterisk
                />
            )}
            {pruneScheduleType !== 'custom' && pruneScheduleType !== 'manual' && (
                <Paper withBorder p="xs" bg="var(--mantine-color-gray-0)">
                    <Text size="xs" c="dimmed">Resulting Cron: <b>{form.values.pruneSchedule.cron}</b></Text>
                </Paper>
            )}
        </Stack>
    )
